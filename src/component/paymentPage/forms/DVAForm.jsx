
import React from "react";

export default function DVAForm({ order, dva, onUploadDvaReceipt }) {
  function formatNumber(num) {
    if (typeof num !== "number") num = Number(num) || 0;
    return num.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
  }

  return (
    <div id="dva-form" className="payment-form active">
      <h3 className="form-title text-[var(--gold)] mb-4">DVA Transfer</h3>

      <div className="bank-details mb-4 p-4 rounded-md bg-[rgba(0,0,0,0.25)] border-l-4 border-[var(--gold)]">
        <div className="bank-name font-semibold text-[var(--gold)] mb-2">
          Paystack Virtual Account
        </div>
        <div className="account-details text-sm mb-2">
          <div>
            <strong>Bank:</strong> {dva.bank}
          </div>
          <div>
            <strong>Account Number:</strong> {dva.number}
          </div>
          <div>
            <strong>Account Name:</strong> Monness Luxury Hairs
          </div>
          <div>
            <strong>Amount:</strong>{" "}
            <span className="font-semibold">₦{formatNumber(order.price)}</span>
          </div>
          <div>
            <strong>Expires in:</strong>{" "}
            <span
              className={`font-semibold ${
                dva.expiryMinutes === 0 ? "text-red-400" : ""
              }`}
            >
              {dva.expiryMinutes > 0
                ? `${dva.expiryMinutes} minutes`
                : "Expired"}
            </span>
          </div>
        </div>
        <p className="text-sm opacity-80">
          Transfer the exact amount to the virtual account above.
        </p>
      </div>

      <div className="upload-receipt">
        <p className="mb-2">After payment, upload your receipt:</p>
        <button
          onClick={onUploadDvaReceipt}
          className="upload-btn py-2 px-4 rounded-md bg-[var(--gold)] text-black font-semibold"
        >
          Upload Receipt via WhatsApp
        </button>
      </div>
    </div>
  );
}
